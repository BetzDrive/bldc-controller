...
int flashSectorErase(flashsector_t sector) {
   ...
   * others not allowed */
  FLASH->CR &=
      ~(FLASH_CR_SNB_0 | FLASH_CR_SNB_1 | FLASH_CR_SNB_2 | FLASH_CR_SNB_3);
  if (sector & 0x1)
  ...
  if (flashIsErased(flashSectorBegin(sector), flashSectorSize(sector)) == FALSE)
    return FLASH_RETURN_BAD_FLASH; /* Sector is not empty despite the erase
                                      cycle! */
...
bool_t flashIsErased(flashaddr_t address, size_t size) {
  ...
  while (size >= sizeof(flashdata_t)) {
    if (*(volatile flashdata_t *)address !=
        (flashdata_t)(-1)) // flashdata_t being unsigned, -1 is 0xFF..FF
      return FALSE;
...
int flashWrite(flashaddr_t address, const char *buffer, size_t size) {
  ...
  if (alignOffset != 0) {
    ...
    if (chunkSize > size)
      chunkSize = size; // this happens when both address and address + size are
                        // not aligned
