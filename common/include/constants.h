...
namespace motor_driver {
  namespace consts {
    ...
    // How long the comms activity LED stays lit after a packet, in
    // milliseconds.
    ...
    // Hz.
    constexpr unsigned int motor_pwm_clock_freq = 168000000;

    // Hz.
    constexpr unsigned int motor_pwm_cycle_freq = 40000;

    // TIM3 can't handle the full 168MHz like TIM1 (Hz).
    constexpr unsigned int adc_pwm_cycle_freq = motor_pwm_clock_freq/2;
    ...
    constexpr unsigned int velocity_control_freq = (
        current_control_freq / vel_divider);
    constexpr uint8_t pos_divider = 5;
    constexpr unsigned int position_control_freq = (
        current_control_freq / pos_divider);
    ...
    constexpr unsigned int current_control_count_per_motor_cycle = (
        motor_pwm_cycle_freq / current_control_freq);
    ...
    // "double-buffering".
    constexpr size_t ivsense_sample_buf_depth = ivsense_samples_per_cycle * 2;

    // 1 voltage channel, 3 current channels.
    constexpr size_t ivsense_channel_count = 4;

    // Phase A current channel index.
    constexpr size_t ivsense_channel_ia = 0;
    // Phase B current channel index.
    constexpr size_t ivsense_channel_ib = 1;
    // Phase C current channel index.
    constexpr size_t ivsense_channel_ic = 2;
    // Supply voltage channel index.
    constexpr size_t ivsense_channel_vin = 3;
    ...
    constexpr float isense_current_per_count = (
        adc_v_per_count * isense_amp_per_v);
    // Maximum expected current measurement.
    constexpr float isense_current_max = (
        (adc_vref_voltage - isense_v_offset) * isense_amp_per_v);
    // Minimum expected current measurement.
    constexpr float ivsense_current_min = (
        (-isense_v_offset) * isense_amp_per_v);
    // Voltage at zero current.
    constexpr float isense_voltage_zero_current = isense_v_offset;
    // ADC Value zero current is centered on.
    constexpr float isense_count_zero_current = (
        isense_voltage_zero_current / adc_v_per_count);
    ...
    constexpr float vsense_voltage_max = (
        adc_vref_voltage * vsense_voltage_ratio);

    // Actual voltage per ADC count.
    constexpr float vsense_voltage_per_count = (
        vsense_voltage_max / adc_max_value);
    ...
    // Phase A current channel index.
    constexpr size_t recorder_channel_ia = 0;
    // Phase B current channel index.
    constexpr size_t recorder_channel_ib = 1;
    // Phase C current channel index.
    constexpr size_t recorder_channel_ic = 2;
    // Supply voltage channel index.
    constexpr size_t recorder_channel_vin = 3;
    // Rotor position channel index.
    constexpr size_t recorder_channel_rotor_pos = 4;
    // Rotor velocity channel index.
    constexpr size_t recorder_channel_rotor_vel = 5;
    // Extra 1 channel index.
    constexpr size_t recorder_channel_ex1 = 6;
    // Extra 2 channel index.
    constexpr size_t recorder_channel_ex2 = 7;
