...
private:
  ...
  /* Receive DMA buffer */
  Buffer rx_buf_(header_len + max_dg_payload_len + crc_length);
  size_t rx_len_;
  bool rx_error_;
  //uint16_t first_crc_; // REMOVE ONCE CRC WORKS

  /* Transmit DMA buffer */
  Buffer tx_buf_(header_len + max_dg_payload_len + crc_length);
  size_t tx_len_;
