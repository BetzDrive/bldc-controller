...
namespace motor_driver {
  namespace controller {
    ...
    static PID pid_id(
        state::calibration.foc_kp_d, state::calibration.foc_ki_d, 0.0f,
        consts::current_control_interval);
    static PID pid_iq(
        state::calibration.foc_kp_q, state::calibration.foc_ki_q, 0.0f,
        consts::current_control_interval);
    static PID pid_velocity(
        state::calibration.velocity_kp, 0.0f, state::calibration.velocity_kd,
        consts::velocity_control_interval);
    static PID pid_position(
        state::calibration.position_kp, 0.0f, state::calibration.position_kd,
    ...
    static float getEncoderAngleCorrection(float raw_enc_pos) {
      if (state::calibration.enc_ang_corr_scale != 0.0f) {
        return (
            (enc_ang_corr_table(raw_enc_pos) *
    ...
    void initControl() {
      pid_id.setLimits(
          -consts::isense_current_max, consts::isense_current_max);
      pid_iq.setLimits(
          -consts::isense_current_max, consts::isense_current_max);
    ...
    void runInnerControlLoop() {
      ...
      while (true) {
        ...
        // TODO: The flag clear is placed in a bad location... Figure out a
        // cleaner solution.
    ...
    void estimateState() {
      ...
      rolladc.count = (
          (rolladc.count + 1) % consts::ivsense_rolling_average_count);
    ...
    void runPositionControl() {
      if (state::parameters.control_mode == consts::control_mode_position ||
        ...
        state::parameters.torque_sp = pid_position.compute(
            state::results.rotor_pos);
      }
    }
    ...
    void runVelocityControl() {
      if (state::parameters.control_mode == consts::control_mode_velocity ||
        ...
        state::parameters.torque_sp = pid_velocity.compute(
            state::results.hf_rotor_vel);
      }
    }
    ...
    void runCurrentControl() {
      ...
      if (state::parameters.control_mode ==
          consts::control_mode_raw_phase_pwm) {
        /*
         * Directly set PWM duty cycles
         */

        peripherals::gate_driver.setPWMDutyCycle(
            0, state::parameters.phase0 * consts::max_duty_cycle);
        peripherals::gate_driver.setPWMDutyCycle(
            1, state::parameters.phase1 * consts::max_duty_cycle);
        peripherals::gate_driver.setPWMDutyCycle(
            2, state::parameters.phase2 * consts::max_duty_cycle);
      } else {
        /*
         * Run field-oriented control
         */
        float ialpha, ibeta;
        math::transformClarke(
            state::results.ia, state::results.ib, state::results.ic,
            ialpha, ibeta);
        ...
        math::transformInversePark(
            vd_norm, vq_norm, cos_theta, sin_theta, valpha_norm, vbeta_norm);
        ...
        if (state::parameters.gate_active) {
          state::results.duty_a = clamp(
              state::results.duty_a,
              consts::min_duty_cycle,
              consts::max_duty_cycle);
          state::results.duty_b = clamp(
              state::results.duty_b,
              consts::min_duty_cycle,
              consts::max_duty_cycle);
          state::results.duty_c = clamp(
              state::results.duty_c,
              consts::min_duty_cycle,
              consts::max_duty_cycle);
        } else {
