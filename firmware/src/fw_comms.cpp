...
namespace motor_driver {
  namespace comms {
    ...
    size_t commsRegAccessHandler(
      ...
      for (comm_addr_t addr = start_addr;
          addr < start_addr + reg_count; addr++) {
        if (addr >= 0x8000) {
          if (recordings != nullptr) {
            ...
            if (i < rec_size) {
              handleVarAccess(
                  recordings[i], buf, index, buf_size, access_type, errors);
            } else {
          ...
          handleVarAccess(
              state::calibration.enc_ang_corr_table_values[addr - 0x1200],
          ...
          switch (addr) {
            ...
            case 0x100A: // Position Controller Kd
              handleVarAccess(state::calibration.position_kd, buf, index, buf_size, access_type, errors);
              break;
