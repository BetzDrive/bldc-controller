...
namespace motor_driver {
  namespace controller {
    ...
    void SVM::computeDutyCycles(
        float v_alpha, float v_beta, float& dc_a, float& dc_b, float& dc_c) {
      dc_a = (v_alpha) * consts::one_div_sqrt3 + 0.5f;
      dc_b = (
          (-0.5f * v_alpha + consts::sqrt3_div_2 * v_beta) *
          consts::one_div_sqrt3 + 0.5f);
      dc_c = (
          (-0.5f * v_alpha - consts::sqrt3_div_2 * v_beta) *
          consts::one_div_sqrt3 + 0.5f);
