...
bool DRV8312::hasFault() { return !palReadPad(nfault_.port, nfault_.pin); }

bool DRV8312::hasOCTW() { return !palReadPad(noctw_.port, noctw_.pin); }
...
void DRV8312::setPWMDutyCycle(int phase, float duty_cycle) {
  int32_t pulse_width =
      (::lround((1 - duty_cycle) * pwm_driver_->config->period));
  pulse_width = std::max(
