...
def setArmPosition(goal, zero_right, zero_left, pos_right, pos_left, duty_cycle, client):
    ...
    overheated = False
    while (not abs(pos_left - goal_left)/goal_left < target_accurracy) or (not abs(pos_right - goal_right)/goal_right < target_accurracy) :
        try:
...
if __name__ == '__main__':
    ...
    while True: #num_lifts < MAX_CYCLES:
        ...
        # Lift the arm up 
        print("lift")
        pos_right, pos_left = setArmPosition(-2, zero_right, zero_left, pos_right, pos_left, duty_cycle, client)
